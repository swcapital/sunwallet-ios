...
struct BlockchairBlockchainRepository: BlockchainRepository {
    ...
    func balance(for wallet: Wallet) -> AnyPublisher<WalletBalance, Error> {
        ...
        case .eth:
            return ethereumBalance(wallet: wallet)
        default:
...
extension BlockchairBlockchainRepository {
    ...
    private func ethereumBalance(wallet: Wallet) -> AnyPublisher<WalletBalance, Error> {
        let address = "\(host)/ethereum/dashboards/address/\(wallet.address)" + "?erc_20=true"
        let url = URL(string: address)!
        return URLSession.shared.dataTaskPublisher(for: url)
            .extractData()
            .decode(type: BalancesResponse<EthereumData>.self, decoder: decoder)
            .print()
            .map {
                let data = $0.data.first!.value
    ...
    private struct EthereumData: Codable {
        let address: EthereumBalance
        let calls: [EthereumCall]
        let layer2: Layer2?
    ...
    private struct EthereumBalance: Codable {
        let balance: String?
        let balanceUSD: Double
    ...
    private struct EthereumCall: Codable {
        let time: Date
        let value: Double
